...
1. What is SAP BTP?
2. BTP Organizational Structure
3. BTP Pillars
4. SAP Integration Suite — Components
5. SAP Integration Suite — Integration Flow
6. SAP BTP Kyma Runtime — Architecture
7. SAP BTP Kyma Runtime — Practical Flow
8. Kyma vs Cloud Foundry
9. How Everything Fits Together — Big Picture
...
**SAP BTP (Business Technology Platform)** is SAP's **PaaS (Platform as a Service)** — think of it as **SAP's "AWS/Azure/GCP"**.

It brings together everything you need to **develop, integrate, automate, and analyze data** within the SAP ecosystem (and beyond it too).

### 🔑 Analogy

> If **SAP S/4HANA** (the ERP) were a **commercial building**, then **SAP BTP** would be the **land + infrastructure** (roads, power, water, internet) around it, allowing you to build **annexes, parking lots, and shops** without touching the main building.

### 🧹 Key Concept: "Clean Core"

SAP encourages you to **NOT modify the ERP directly**. Instead, use BTP to create **side-by-side extensions**:

| Approach | Analogy |
|----------|---------|
| ❌ Modify the ERP | **Fork** a library and maintain it manually |
| ✅ Extension via BTP | Create a **plugin/extension** that consumes the library's API |
...
### Non-SAP World Equivalence

| SAP BTP | AWS Equivalent |
|---------|----------------|
| **Global Account** | AWS Organization |
| **Directory** | OU (Organizational Unit) |
| **Subaccount** | Individual AWS Account |
...
| Pillar | What it does | Highlights |
|--------|--------------|------------|
| 🗄️ **Database & Data Mgmt** | Data storage and management | HANA Cloud, DataSphere, Data Intelligence |
| ⚙️ **App Dev & Integration** | App development and integration | Integration Suite, Kyma, Cloud Foundry |
| 📊 **Analytics** | Data analysis and BI | SAP Analytics Cloud |
| 🤖 **AI / ML** | Artificial intelligence and automation | AI Core, RPA, GenAI |
...
> If your systems were **islands**, the Integration Suite would be the **network of bridges, ferries, and submarine cables** connecting all of them.

It's SAP's **iPaaS (Integration Platform as a Service)** — equivalent to **Apache Kafka + MuleSoft + Kong API Gateway** — all in one.
...
### Mapping to the Non-SAP World

| SAP Integration Suite | Non-SAP Equivalent |
|-----------------------|--------------------|
...
| Open Connectors | Zapier / Workato (pre-built connectors) |
| Integration Advisor | AI that suggests data mappings |
| Edge Integration Cell | On-prem managed agent (like Azure Arc) |
...
| Component | Role in the Flow | Protocol |
|-----------|-----------------|----------|
| **Cloud Integration** | Transforms, routes, and maps data between SAP and Salesforce | OData / REST / API |
| **API Management** | Gateway for mobile apps and B2B partners | HTTPS / EDI |
| **Event Mesh** | Async Pub/Sub for microservices and IoT | Events / Telemetry |
...
> If **Kubernetes** were an **empty lot with infrastructure**, **Kyma** would be a **ready-made industrial park** — it already has a gatehouse (API Gateway), alarm system (observability), mail service (eventing), and a direct connection to the "SAP factory" next door.

**Kyma = Kubernetes managed by SAP**, equivalent to **EKS/AKS/GKE**, but with "batteries included" for the SAP ecosystem.
...
|-------|------------|---------|
| **Your Workloads** | Microservices (Node.js, Go, Java), Serverless Functions | Your business code |
| **Kyma Components** | API Gateway (Istio), Eventing (NATS), Service Catalog, Observability | Pre-installed infrastructure |
| **BTP Services** | HANA Cloud, XSUAA, Destination, Connectivity | Platform managed services |
| **SAP S/4HANA** | OData / REST APIs | ERP and backend systems |
...
|------|--------|----------------|
| 1️⃣ **Provision** | Create Kyma environment | BTP Cockpit |
| 2️⃣ **Develop** | Write code in any language | VS Code, IntelliJ, etc. |
| 3️⃣ **Containerize** | Package into Docker containers | `docker build` / `docker push` |
| 4️⃣ **Deploy** | Apply to the K8s cluster | `kubectl apply -f` / CI/CD |
| 5️⃣ **Connect** | Receive SAP events automatically | Kyma Eventing |
| 6️⃣ **Expose** | Expose APIs with authentication | API Gateway (Istio) |
| 7️⃣ **Monitor** | Dashboards for logs, metrics, and traces | Built-in Observability |

---

# 8. Kyma vs Cloud Foundry

The two **runtimes** of SAP BTP side by side:

| Feature | ⚙️ Kyma | ☁️ Cloud Foundry |
|---------|---------|-------------------|
| **Foundation** | Kubernetes | Cloud Foundry (PaaS) |
| **Control** | Full (YAML, Helm, kubectl) | Abstracted (`cf push`) |
| **Learning curve** | Steeper (requires K8s knowledge) | Gentler (more opinionated) |
| **Flexibility** | Very high | Medium |
| **Ideal for** | Microservices, event-driven | Traditional apps, APIs |
| **Analogy** | EKS / AKS / GKE | Heroku / Railway |

### When to use which?

```text
Kyma  → You want full control, already know Kubernetes,
         need event-driven architecture or complex microservices.

Cloud Foundry → You want simplicity, fast deploy with `cf push`,
                 ideal for traditional apps and simple REST APIs.
```
...
|-----------|--------------|
| **SAP BTP** | SAP's multi-cloud PaaS — the "land" where you build, integrate, and analyze everything. |
| **Integration Suite** | iPaaS that connects SAP and non-SAP via APIs, events, and flows — the "mail + bridges" between systems. |
| **Kyma** | Managed Kubernetes with "SAP batteries" — where you run microservices and serverless functions. |
...
| Resource | Link |
|----------|------|
| SAP BTP Overview 2025 | [Official PDF](https://assets.dm.ux.sap.com/sap-user-groups/pdfs/250218_sap_business_technoloy_platform_2025_overview.pdf) |
...
| Comprehensive Guide to SAP BTP | [sapa2z.com](https://sapa2z.com/comprehensive-guide-to-sap-btp/) |
| SAP BTP Architecture | [kaartech.com](https://www.kaartech.com/blogs/sap-btp-architecture/) |
...
> *Presentation generated from the SAP BTP, Integration Suite & Kyma guide for non-SAP developers.*
