...
"!
"! Used in the async flow (bgPF). Each worker records successes and
"! errors in BAL for clean-core observability and audit.
CLASS zcl_equi_load_sink_applog DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    CONSTANTS c_object    TYPE c LENGTH 20 VALUE 'ZEQUI_LOAD'.
...
CLASS zcl_equi_load_sink_applog IMPLEMENTATION.
  ...
  METHOD write_bal.
    ...
    " released cl_bali_* API for BAL (Clean Core Level 1)
    TRY.
        ...
        LOOP AT it_result INTO DATA(ls_r).
          ...
          DATA(lv_type) = COND symsgty(
                           WHEN ls_r-status = 'S' THEN 'S'
                                                  ELSE 'E' ).

          DATA(lo_msg) = cl_bali_free_text_setter=>create(
                          severity = lv_type
                          text     = |{ ls_r-ext_id } { ls_r-equipment } { ls_r-message }| ).
      ...
      CATCH cx_bali_runtime.
        " In production: forward to alternative telemetry.
