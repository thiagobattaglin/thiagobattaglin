"! <p class="shorttext synchronized">Unit tests for RAP BO ZR_LOAD_REQ</p>
...
CLASS ltcl_load_req IMPLEMENTATION.
  ...
  METHOD class_setup.
    cds_env = cl_cds_test_environment=>create(
      i_for_entities = VALUE #( ( 'ZR_LOAD_REQ' ) ( 'ZR_LOAD_ITM' ) ) ).
  ENDMETHOD.
  ...
  METHOD create_request.
    MODIFY ENTITIES OF zr_load_req
      ENTITY LoadReq
    ...
    cl_abap_unit_assert=>assert_initial( act = failed msg = 'Create must not fail' ).
  ...
  METHOD default_mode_is_async.
    ...
    READ ENTITIES OF zr_load_req
      ENTITY LoadReq FIELDS ( Mode )
      WITH VALUE #( ( RequestUuid = lv_uuid ) )
      RESULT DATA(reqs).

    cl_abap_unit_assert=>assert_equals(
      exp = 'A' act = reqs[ 1 ]-Mode
      msg = 'Missing Mode must default to A' ).
  ENDMETHOD.
  ...
  METHOD default_status_is_new.
    ...
    READ ENTITIES OF zr_load_req
      ENTITY LoadReq FIELDS ( Status )
      WITH VALUE #( ( RequestUuid = lv_uuid ) )
      RESULT DATA(reqs).

    cl_abap_unit_assert=>assert_equals(
      exp = 'N' act = reqs[ 1 ]-Status
      msg = 'Missing Status must default to N' ).
  ENDMETHOD.
