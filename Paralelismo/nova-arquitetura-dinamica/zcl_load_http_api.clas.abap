"! <p class="shorttext synchronized">HTTP API — Dynamic Load (Clean Core Add-on)</p>
"!
"! Contract (JSON):
"!   POST /load
"!   {
...
"!   }
"!
"! Supported object_type values are dispatched by ZCL_LOAD_ADAPTER_FACTORY.
"! No dynamic CALL FUNCTION — every dispatch resolves to a static class.
CLASS zcl_load_http_api DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
  ...
  PRIVATE SECTION.
    ...
    TYPES:
      BEGIN OF ty_request,
        mode        TYPE string,
        worker_rows TYPE i,
        items       TYPE zcl_load_dto=>tt_input,
      END OF ty_request.

    TYPES:
      BEGIN OF ty_response_sync,
        results TYPE zcl_load_dto=>tt_result,
    ...
    METHODS write_response_sync
      IMPORTING io_response TYPE REF TO if_web_http_response
                it_result   TYPE zcl_load_dto=>tt_result
      RAISING   cx_web_message_error.
...
CLASS zcl_load_http_api IMPLEMENTATION.
  ...
  METHOD if_http_service_extension~handle_request.
    ...
    TRY.
        ...
        IF ls_req-items IS INITIAL.
          write_error( io_response = response iv_status = 400
                       iv_message  = `items must not be empty` ).
          RETURN.
        ENDIF.

        LOOP AT ls_req-items INTO DATA(ls_item) WHERE object_type IS INITIAL.
          write_error( io_response = response iv_status = 400
                       iv_message  = |Missing object_type at ext_id { ls_item-ext_id }| ).
          RETURN.
        ENDLOOP.

        DATA(lo_source) = CAST zif_load_source( NEW zcl_load_src_http( ls_req-items ) ).

        IF ls_req-mode = `sync`.

          DATA(lo_sink_sync) = NEW zcl_load_sink_memory( ).
          DATA(lo_orch_sync) = NEW zcl_load_orchestrator(
                                io_source      = lo_source
                                io_sink        = CAST zif_load_sink( lo_sink_sync )
                                iv_worker_rows = ls_req-worker_rows ).
          write_response_sync( io_response = response
                               it_result   = lo_orch_sync->run_sync( ) ).

        ELSE.

          DATA(lo_sink_async) = CAST zif_load_sink( NEW zcl_load_sink_applog( ) ).
          DATA(lo_orch_async) = NEW zcl_load_orchestrator(
                                  io_source      = lo_source
                                  io_sink        = lo_sink_async
                                  iv_worker_rows = ls_req-worker_rows ).
          DATA(lv_workers) = lo_orch_async->run( ).

          write_response_async( io_response = response
      ...
      CATCH cx_web_message_error INTO DATA(lx_msg).
        write_error( io_response = response iv_status = 400 iv_message = lx_msg->get_text( ) ).

      CATCH cx_root INTO DATA(lx_root).
        write_error( io_response = response iv_status = 500 iv_message = lx_root->get_text( ) ).
  ...
  METHOD write_response_async.
    DATA(ls_body) = VALUE ty_response_async( accepted = iv_accepted workers = iv_workers mode = 'async' ).
    DATA(lv_json) = xco_cp_json=>data->from_abap( ls_body
      )->apply( VALUE #( ( xco_cp_json=>transformation->pascal_case_to_underscore ) )
      )->to_string( ).
