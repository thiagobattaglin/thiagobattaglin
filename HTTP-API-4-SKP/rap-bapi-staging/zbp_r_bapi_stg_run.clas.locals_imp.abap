...
*"* declarations
CLASS lhc_bapi_stg_run DEFINITION INHERITING FROM cl_abap_behavior_handler.
  ...
  PRIVATE SECTION.
    ...
    METHODS persist_run
      IMPORTING is_outcome     TYPE zcl_bapi_stg_dispatcher=>ty_outcome
                iv_wt          TYPE i
                iv_wr          TYPE i
                iv_status      TYPE c
...
CLASS lhc_bapi_stg_run IMPLEMENTATION.
  ...
  METHOD submit.
    ...
    LOOP AT keys ASSIGNING FIELD-SYMBOL(<key>).
      ...
      TRY.
          DATA(lo_dispatcher) = NEW zcl_bapi_stg_dispatcher( ).
          ...
          DATA(ls_outcome) = lo_dispatcher->dispatch( iv_run_uuid = lv_uuid
                                                     iv_json     = lv_payload ).

          persist_run( is_outcome    = ls_outcome
        ...
        CATCH cx_root INTO DATA(lx_err).
          ...
          persist_run(
            is_outcome    = VALUE #( run_uuid = lv_uuid
  ...
  METHOD persist_run.
    MODIFY ENTITIES OF zr_bapi_stg_run IN LOCAL MODE
      ENTITY BapiRun
        CREATE FIELDS ( RunUuid BapiName ExecMode
                        WorkerThreads WorkerRows
                        Accepted Workers Status ErrorText )
        WITH VALUE #( ( %cid          = |CID_{ is_outcome-run_uuid }|
                        RunUuid       = is_outcome-run_uuid
