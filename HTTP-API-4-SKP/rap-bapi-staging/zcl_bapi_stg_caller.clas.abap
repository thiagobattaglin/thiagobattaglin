CLASS zcl_bapi_stg_caller DEFINITION
  PUBLIC
  CREATE PUBLIC.

* Worker side. Reads its slice from the staging tables (no JSON at all),
* rehydrates documents, executes the BAPI, and commits per document.

  PUBLIC SECTION.

    TYPES ty_fields    TYPE zcl_bapi_stg_dispatcher=>tt_fields.
    TYPES ty_structs   TYPE zcl_bapi_stg_dispatcher=>tt_structs.
    TYPES ty_document  TYPE zcl_bapi_stg_dispatcher=>ty_document.
    TYPES tt_documents TYPE zcl_bapi_stg_dispatcher=>tt_documents.
    ...
    "! Loads documents [iv_doc_from..iv_doc_to] for iv_run_uuid from the
    "! staging tables and executes them in sequence.
    METHODS process_range
    ...
    METHODS call_document
      IMPORTING iv_doc_seq       TYPE i
                is_document      TYPE ty_document
...
CLASS zcl_bapi_stg_caller IMPLEMENTATION.
  ...
  METHOD process_range.
    DATA(lt_docs) = load_documents( iv_run_uuid = iv_run_uuid
                                    iv_doc_from = iv_doc_from
                                    iv_doc_to   = iv_doc_to ).
    DATA lv_seq TYPE i.
    lv_seq = iv_doc_from - 1.
    LOOP AT lt_docs INTO DATA(ls_doc).
      lv_seq = lv_seq + 1.
      APPEND call_document( iv_doc_seq  = lv_seq
                            is_document = ls_doc ) TO rt_results.
    ENDLOOP.
  ENDMETHOD.

  METHOD load_documents.

* Reads the item rows for the range and rehydrates in-memory documents.
* One SELECT hits the staging item table with a range restriction that
* matches the primary key prefix (run_uuid, doc_seq) - index-friendly.

    DATA lt_items TYPE STANDARD TABLE OF zbapi_stg_item WITH EMPTY KEY.

    SELECT run_uuid, doc_seq, section, param_seq, field_seq,
           param_name, field_name, field_value
      FROM zbapi_stg_item
      WHERE run_uuid = @iv_run_uuid
        AND doc_seq  BETWEEN @iv_doc_from AND @iv_doc_to
      ORDER BY doc_seq, section, param_seq, field_seq
    ...
    DATA lv_current_doc TYPE i VALUE 0.
    DATA ls_document    TYPE zcl_bapi_stg_dispatcher=>ty_document.

    DATA lv_h_param     TYPE i VALUE 0.
    DATA lv_i_param     TYPE i VALUE 0.

    LOOP AT lt_items ASSIGNING FIELD-SYMBOL(<ls_it>).

      IF <ls_it>-doc_seq <> lv_current_doc.
        IF lv_current_doc <> 0.
          APPEND ls_document TO rt_docs.
        ENDIF.
        CLEAR ls_document.
        lv_current_doc = <ls_it>-doc_seq.
        lv_h_param = 0.
        lv_i_param = 0.
      ENDIF.

      IF <ls_it>-section = 'H'.
        IF <ls_it>-param_seq <> lv_h_param.
          APPEND VALUE zcl_bapi_stg_dispatcher=>ty_struct(
            value = <ls_it>-param_name ) TO ls_document-heders_values.
          lv_h_param = <ls_it>-param_seq.
        ENDIF.
        DATA(lv_idx_h) = lines( ls_document-heders_values ).
        APPEND VALUE zcl_bapi_stg_dispatcher=>ty_field(
          name  = <ls_it>-field_name
          value = <ls_it>-field_value )
          TO ls_document-heders_values[ lv_idx_h ]-fields.
      ELSE.
        IF <ls_it>-param_seq <> lv_i_param.
          APPEND VALUE zcl_bapi_stg_dispatcher=>ty_struct(
            value = <ls_it>-param_name ) TO ls_document-items_values.
          lv_i_param = <ls_it>-param_seq.
        ENDIF.
        DATA(lv_idx_i) = lines( ls_document-items_values ).
        APPEND VALUE zcl_bapi_stg_dispatcher=>ty_field(
          name  = <ls_it>-field_name
          value = <ls_it>-field_value )
          TO ls_document-items_values[ lv_idx_i ]-fields.
      ENDIF.
