CLASS zcl_http_bapi_hyb_caller DEFINITION
  PUBLIC
  CREATE PUBLIC.

* Worker-side orchestrator for the http-api-hybrid project.
*
* Uses zcl_http_bapi_hyb_json_parser (kernel sXML + CTF id) to
* deserialize a chunk and then dispatches each document dynamically to
* the requested BAPI with per-document commit/rollback.
*
* Clean Core caveat: FUNCTION_IMPORT_INTERFACE + dynamic CALL FUNCTION
* are NOT released for ABAP Cloud public. Intended for on-premise /
* embedded Steampunk / private cloud, where dynamic classic BAPI
...
CLASS zcl_http_bapi_hyb_caller IMPLEMENTATION.
  ...
  METHOD process_chunk_json.
    TRY.
        DATA(lt_docs) = deserialize_documents( iv_chunk ).
  ...
  METHOD deserialize_documents.
* Chunk shape may be:
*   - JSON array from the lexical splitter:  [{doc},{doc}]
*   - single-object POST from the client   :  { heders_values, items_values }
*   - full bulk wrapper                     :  { documents:[...] }
* CTF id handles all three: array -> tt_documents; object -> ty_wrapper.

    DATA(lv_trim) = shift_left( shift_right( iv_chunk ) ).
    ...
    IF lv_trim(1) = '['.
      zcl_http_bapi_hyb_json_parser=>deserialize(
        EXPORTING iv_json = lv_trim
        CHANGING  cs_data = rt_docs ).
      RETURN.
    ENDIF.
    ...
    zcl_http_bapi_hyb_json_parser=>deserialize(
      EXPORTING iv_json = lv_trim
      CHANGING  cs_data = ls_wrap ).

    IF ls_wrap-documents IS NOT INITIAL.
      rt_docs = ls_wrap-documents.
    ELSEIF ls_wrap-heders_values IS NOT INITIAL OR ls_wrap-items_values IS NOT INITIAL.
      APPEND VALUE ty_document( heders_values = ls_wrap-heders_values
                                items_values  = ls_wrap-items_values ) TO rt_docs.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD extract_bapi_messages.
    ...
    ELSEIF lo_desc->kind = cl_abap_typedescr=>kind_struct.
      ...
      IF sy-subrc = 0. ls_msg2-message = <fs_v2>. ENDIF.
      IF ls_msg2-type IS NOT INITIAL.
        APPEND ls_msg2 TO et_messages.
        IF ls_msg2-type = 'E' OR ls_msg2-type = 'A' OR ls_msg2-type = 'X'.
          ev_success = abap_false.
        ENDIF.
      ENDIF.
    ENDIF.
