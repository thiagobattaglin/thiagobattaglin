CLASS zcl_http_bapi_hyb_lex_splitter DEFINITION
  PUBLIC
  CREATE PUBLIC.

* Lexical splitter for a JSON payload of shape
*   { ..., "documents": [ {doc1}, {doc2}, ... ], ... }
*
* Splits documents[] textually in a single O(strlen) pass while
* tracking brace depth and JSON string escape state. Emits valid JSON
* array substrings ("[{doc},{doc}]") ready to be fed to a worker.
*
* Contains all the hand-rolled parser surface area in one testable class.
...
CLASS zcl_http_bapi_hyb_lex_splitter IMPLEMENTATION.
