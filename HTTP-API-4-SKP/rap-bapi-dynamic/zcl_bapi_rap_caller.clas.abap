CLASS zcl_bapi_rap_caller DEFINITION
  PUBLIC
  CREATE PUBLIC.

  PUBLIC SECTION.

    TYPES ty_fields    TYPE zcl_bapi_rap_dispatcher=>tt_fields.
    TYPES ty_structs   TYPE zcl_bapi_rap_dispatcher=>tt_structs.
    TYPES ty_document  TYPE zcl_bapi_rap_dispatcher=>ty_document.
    TYPES tt_documents TYPE zcl_bapi_rap_dispatcher=>tt_documents.
    ...
    "! Processes one chunk (document batch) with commit per document.
    METHODS process_chunk
      IMPORTING it_documents      TYPE tt_documents
      RETURNING VALUE(rt_results) TYPE tt_doc_results.

    "! Executes one document: introspection + bind + CALL FUNCTION + commit/rollback.
...
CLASS zcl_bapi_rap_caller IMPLEMENTATION.
  ...
  METHOD call_document.
    ...
    " ---- IMPORTING / CHANGING structures from heders_values ------------------
    ...
    " ---- TABLES rows from items_values ---------------------------------------
    LOOP AT is_document-items_values INTO DATA(ls_itm).
      DATA(ls_meta_t) = find_param( ls_itm-value ).
      IF ls_meta_t-name IS INITIAL OR ls_meta_t-type IS NOT BOUND.
        CONTINUE.
      ENDIF.
      IF ls_meta_t-kind <> c_kind_table.
        CONTINUE.
      ENDIF.

      DATA(lr_tab) = get_or_create_table_ref( EXPORTING is_meta    = ls_meta_t
                                              CHANGING  ct_buckets = lt_bucket ).
      append_row( it_fields = ls_itm-fields
                  ir_table  = lr_tab ).
    ENDLOOP.

    LOOP AT lt_bucket INTO DATA(ls_bucket).
      INSERT VALUE #( name  = ls_bucket-param_name
                      kind  = abap_func_tables
                      value = ls_bucket-table_ref ) INTO TABLE lt_ptab.
    ENDLOOP.

    " ---- Auto-bind RETURN (TABLES or EXPORTING) ------------------------------
  ...
  METHOD get_or_create_table_ref.
    READ TABLE ct_buckets WITH KEY param_name = is_meta-name
                          ASSIGNING FIELD-SYMBOL(<ls_b>).
    IF sy-subrc = 0.
    ...
    APPEND VALUE ty_rows_bucket( param_name = is_meta-name
                                 table_ref  = rr_data ) TO ct_buckets.
  ENDMETHOD.
  ...
  METHOD append_row.
    ...
    fill_structure( it_fields = it_fields
                    ir_target = lr_row ).
  ...
  METHOD extract_bapi_messages.
    ...
    ELSEIF lo_desc->kind = cl_abap_typedescr=>kind_struct.
      DATA ls_smsg TYPE ty_message.
      ASSIGN COMPONENT 'TYPE'    OF STRUCTURE <fs_return> TO FIELD-SYMBOL(<fs_sv>).
      IF sy-subrc = 0. ls_smsg-type = <fs_sv>. ENDIF.
      ASSIGN COMPONENT 'ID'      OF STRUCTURE <fs_return> TO <fs_sv>.
      IF sy-subrc = 0. ls_smsg-id = <fs_sv>. ENDIF.
      ASSIGN COMPONENT 'NUMBER'  OF STRUCTURE <fs_return> TO <fs_sv>.
      IF sy-subrc = 0. ls_smsg-number = <fs_sv>. ENDIF.
      ASSIGN COMPONENT 'MESSAGE' OF STRUCTURE <fs_return> TO <fs_sv>.
      IF sy-subrc = 0. ls_smsg-message = <fs_sv>. ENDIF.
      IF ls_smsg-type IS NOT INITIAL OR ls_smsg-message IS NOT INITIAL.
        APPEND ls_smsg TO et_messages.
        IF ls_smsg-type = 'E' OR ls_smsg-type = 'A' OR ls_smsg-type = 'X'.
          ev_success = abap_false.
        ENDIF.
      ENDIF.
    ENDIF.
