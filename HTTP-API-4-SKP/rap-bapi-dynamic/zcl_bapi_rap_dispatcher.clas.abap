CLASS zcl_bapi_rap_dispatcher DEFINITION
  PUBLIC
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    "! Parse + defaults + worker calculation + async dispatch (fire-and-forget).
    METHODS dispatch
  ...
  PROTECTED SECTION.
    ...
    "! Injectable hook: by default dispatches via CALL FUNCTION STARTING NEW TASK.
    "! Overridden by tests to avoid real RFC dispatch.
...
CLASS zcl_bapi_rap_dispatcher IMPLEMENTATION.
  ...
  METHOD split_documents.
    ...
    DO iv_workers TIMES.
      ...
      IF lt_chunk IS NOT INITIAL.
        APPEND /ui2/cl_json=>serialize( data        = lt_chunk
                                        pretty_name = /ui2/cl_json=>pretty_mode-none )
               TO rt_chunks.
      ENDIF.
  ...
  METHOD dispatch_chunks.
    ...
    LOOP AT it_chunks ASSIGNING FIELD-SYMBOL(<lv_chunk>).
      ...
      TRY.
          CALL FUNCTION 'Z_BAPI_RAP_WORKER'
            ...
            EXPORTING
              ...
              iv_chunk     = <lv_chunk>.
        CATCH cx_root.
          " Transient dispatch failure does not invalidate remaining workers.
      ENDTRY.
    ENDLOOP.
  ENDMETHOD.
