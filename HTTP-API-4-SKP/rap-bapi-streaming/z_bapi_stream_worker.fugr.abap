FUNCTION-POOL z_bapi_stream_worker.
* Function group for RFC-enabled FM Z_BAPI_STREAM_WORKER used by
* zcl_bapi_stream_dispatcher via
*     CALL FUNCTION 'Z_BAPI_STREAM_WORKER' STARTING NEW TASK ...
*     DESTINATION IN GROUP DEFAULT.


FUNCTION z_bapi_stream_worker.
*"---------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
* NOTE: this FM MUST be flagged as Remote-Enabled Module (SE37).
  ...
  TRY.
      DATA(lo_caller) = NEW zcl_bapi_stream_caller( iv_bapi_name ).
