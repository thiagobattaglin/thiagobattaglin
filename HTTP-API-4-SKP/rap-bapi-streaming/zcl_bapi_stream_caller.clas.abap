CLASS zcl_bapi_stream_caller DEFINITION
  PUBLIC
  CREATE PUBLIC.

* Streaming worker side. Only the chunk (already small) is deserialized
* here, so /ui2/cl_json runs against O(chunk_size) not the whole payload.
  ...
  PUBLIC SECTION.
    ...
    "! Accepts either an object payload (single doc / documents[]) or a
    "! raw JSON array chunk emitted by the lexical splitter.
...
CLASS zcl_bapi_stream_caller IMPLEMENTATION.
...
* Chunk shape from lexical splitter is a JSON array [ {doc}, {doc}, ... ]
* Chunk shape from raw client POST may be either { documents:[...] } or
* a single { heders_values, items_values }. All three are handled here.
    ...
    IF lv_trim(1) = '['.
      /ui2/cl_json=>deserialize(
        EXPORTING json        = lv_trim
                  pretty_name = /ui2/cl_json=>pretty_mode-none
        CHANGING  data        = rt_docs ).
      RETURN.
    ENDIF.
    ...
    ELSEIF ls_wrap-heders_values IS NOT INITIAL OR ls_wrap-items_values IS NOT INITIAL.
      APPEND VALUE ty_document( heders_values = ls_wrap-heders_values
                                items_values  = ls_wrap-items_values )
             TO rt_docs.
    ENDIF.
  ENDMETHOD.
