CLASS zcl_http_bapi_dyn DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
...
CLASS zcl_http_bapi_dyn IMPLEMENTATION.
  ...
  METHOD if_http_service_extension~handle_request.
    ...
    TRY.
        ...
        response->set_status(
          i_code   = COND #( WHEN ls_outcome-mode = zcl_bapi_dyn_dispatcher=>c_mode_sync THEN 200 ELSE 202 )
          i_reason = COND #( WHEN ls_outcome-mode = zcl_bapi_dyn_dispatcher=>c_mode_sync THEN 'OK' ELSE 'Accepted' ) ).
        response->set_header_field( i_name = 'Content-Type' i_value = 'application/json' ).
