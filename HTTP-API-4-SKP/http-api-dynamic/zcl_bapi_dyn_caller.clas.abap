CLASS zcl_bapi_dyn_caller DEFINITION
  PUBLIC
  CREATE PUBLIC.

  PUBLIC SECTION.

    TYPES ty_fields    TYPE zcl_bapi_dyn_dispatcher=>tt_fields.
    TYPES ty_structs   TYPE zcl_bapi_dyn_dispatcher=>tt_structs.
    TYPES ty_document  TYPE zcl_bapi_dyn_dispatcher=>ty_document.
    TYPES tt_documents TYPE zcl_bapi_dyn_dispatcher=>tt_documents.
...
CLASS zcl_bapi_dyn_caller IMPLEMENTATION.
  ...
  METHOD call_document.
    ...
    " ---- Auto-bind RETURN (if BAPI exposes one, TABLES or EXPORTING) ---------
  ...
  METHOD extract_bapi_messages.
    ...
    " Treat as table of BAPIRET-like or a single structure.
