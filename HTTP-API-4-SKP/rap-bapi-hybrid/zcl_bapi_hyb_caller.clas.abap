CLASS zcl_bapi_hyb_caller DEFINITION
  PUBLIC
  CREATE PUBLIC.

* Worker-side orchestrator. Uses zcl_bapi_hyb_json_parser (kernel CTF)
* to deserialize a chunk and then dispatches each document dynamically
* to the requested BAPI with per-document commit/rollback.
*
* Clean Core caveat: FUNCTION_IMPORT_INTERFACE and dynamic CALL FUNCTION
* are NOT released for ABAP Cloud (Steampunk public). This artifact is
* intended for on-premise / embedded Steampunk / private cloud where
* dynamic classic BAPI dispatch is still allowed.
...
CLASS zcl_bapi_hyb_caller IMPLEMENTATION.
...
* Chunk shape may be:
*   - a JSON array from the lexical splitter:  [{doc},{doc}]
*   - a single object POST from the client   :  { heders_values, items_values }
*   - a bulk wrapper                          :  { documents:[...] }
* CTF id maps all three by using a wrapper type with both single and
* bulk members plus a fallback to a raw array.
