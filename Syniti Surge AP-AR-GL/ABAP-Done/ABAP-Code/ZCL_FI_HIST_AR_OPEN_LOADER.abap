"! AR Open Items Loader
"! Posts individual AR open items into S/4HANA via Clean Core wrapper
"! Uses ZIF_FI_ACC_DOC_SERVICE (injected) → items visible in FBL5N
CLASS zcl_fi_hist_ar_open_loader DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    METHODS constructor
      IMPORTING
        it_items       TYPE zcl_fi_hist_types=>ty_ar_open_items
  ...
  PRIVATE SECTION.
    DATA mt_items       TYPE zcl_fi_hist_types=>ty_ar_open_items.
    ...
    METHODS post_single_item
      IMPORTING is_item         TYPE zcl_fi_hist_types=>ty_ar_open_item
...
CLASS zcl_fi_hist_ar_open_loader IMPLEMENTATION.
  ...
  METHOD zif_fi_hist_loader~validate.
    ...
    LOOP AT mt_items ASSIGNING FIELD-SYMBOL(<item>).
      ...
      IF <item>-kunnr IS INITIAL.
        add_log( iv_msgty = 'E' iv_message = |Item { lv_idx }: Customer number is mandatory| ).
        rv_valid = abap_false.
      ENDIF.
      ...
      IF NOT line_exists( mt_bp_mapping[ legacy_id = <item>-kunnr bp_type = 'CU' ] ).
        add_log( iv_msgty = 'E'
                 iv_message = |Item { lv_idx }: No BP mapping for customer { <item>-kunnr }| ).
        rv_valid = abap_false.
      ENDIF.
    ENDLOOP.
  ENDMETHOD.
  ...
  METHOD post_single_item.
    ...
    DATA lt_accountar TYPE STANDARD TABLE OF bapiacar09.
    ...
    DATA(ls_header) = VALUE bapiache09(
      ...
      header_txt = |HIST_AR { is_item-belnr }/{ is_item-gjahr }|
    ...
    APPEND VALUE bapiacar09(
      ...
      customer   = resolve_bp( is_item-kunnr )
      comp_code  = is_item-bukrs
    ...
    " --- Currency Amounts ---
    " GL line (opposite sign — credit side of recon account)
    APPEND VALUE bapiaccr09(
      itemno_acc = '0000000001'
      currency   = is_item-waers
      curr_type  = '00'
      amt_doccur = COND #( WHEN is_item-shkzg = 'S'
    ...
    DATA(ls_doc_result) = COND #(
      ...
      THEN mo_doc_service->check_document(
             ...
             it_accountreceivable = lt_accountar
  ...
  METHOD resolve_bp.
    rv_bp = VALUE #( mt_bp_mapping[ legacy_id = iv_kunnr bp_type = 'CU' ]-bp_number OPTIONAL ).
  ENDMETHOD.
